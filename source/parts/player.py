...
class player(pygame.sprite.Sprite):
    ...
    def get_state(self, normalize=True):
        """
        返回 player 的低维状态字典，方便 RL 环境读取。
        normalize=True 时把位置和角度归一化到近似 [0,1] / [-1,1] 范围。
        """
        raw_x = self.x
        raw_y = self.y
        theta = self.theta
        can_fire = 1 if self.can_fire else 0
        dead = 1 if self.dead else 0
        rounds_count = len(self.rounds)
        # 新增状态
        is_moving = 1 if self.moving else 0
        not_stuck = 1 if self.not_stuck else 0

        if normalize:
            max_x = C.MOTION_CALC_SCALE * C.COLUMN_NUM * C.BLOCK_SIZE
            max_y = C.MOTION_CALC_SCALE * C.ROW_NUM * C.BLOCK_SIZE
            nx = float(raw_x) / max(1.0, max_x)
            ny = float(raw_y) / max(1.0, max_y)
            sin_theta = float(numpy.sin(theta))
            cos_theta = float(numpy.cos(theta))
            if self.can_fire:
                cooldown = 0.0
            else:
                cooldown = min(1.0, float(self.arena.clock - self.fire_cool_down_timer) / max(1.0, C.FIRE_COOL_DOWN))
            return {
                'name': int(self.name),
                'x': nx,
                'y': ny,
                'sin_theta': sin_theta,
                'cos_theta': cos_theta,
                'can_fire': float(can_fire),
                'cooldown': cooldown,
                'rounds_count': int(rounds_count),
                'dead': int(dead),
                'moving': is_moving,       # 新增
                'not_stuck': not_stuck     # 新增
            }
        else:
            return {
                'name': int(self.name),
